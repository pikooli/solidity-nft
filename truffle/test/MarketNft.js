  ...
  it(
    ...
    () =>
      MarketNft.deployed().then(async (instanceMarketNft) => {
        ...
        assert.equal(transactionListing.price, 0);
        assert.equal(transactionListing.seller, 0);
      })
  );

  it(
    "Given a Nft have been sold" +
      "When we check our sold" +
      "Then it should return the sold of the account",
    () =>
      MarketNft.deployed().then(async (instanceMarketNft) => {
        const transaction = await instanceMarketNft.getProceeds(accounts[0]);
        assert.equal(transaction.words[0], newPrice);
      })
  );
});
